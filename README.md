...
| **동일 cwd의 OMO 세션** | 프로세스 매칭에 더해 현재 화면의 고유 문구로 재개 세션을 식별해 여러 OMO 대화를 각각 올바른 GUI에 연결 | 최신 로그 하나를 추측하면 대화가 섞일 수 있음 |
...
- 같은 cwd에서 OMO를 여러 개 실행하거나 오래된 대화를 재개해도, pane별 프로세스 세션을 먼저 확정하고 재개 프로세스는 현재 화면과 로그 tail의 고유 문구를 대조해 각 GUI를 분리한다. 하나로 확정할 수 없으면 다른 대화를 추측해 보여주지 않는다
