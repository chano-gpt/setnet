// Bundled slash-command fallback catalogs, keyed by Herdr's detected agent type (`pane.agent`).
...
// curated as mobile references. They are partial by design: the UI inserts every bundled command
// for review and never treats these arrays as live proof that the current runtime supports it.
//
...
//
// To regenerate: verify the exact runtime/docs version and replace the relevant array.
...
// ── Codex ────────────────────────────────────────────────────────────────────
export const CODEX: readonly AgentCommand[] = [
...
export const OPENCODE: readonly AgentCommand[] = [
